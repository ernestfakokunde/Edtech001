...
import {
  addMyCourse, claimMission, deleteAccount, errorMessage, getCurrentProfile, getMyCourses, getMyMissions, getReferralInfo, redeemPromoCode, removeMyCourse, saveMySchool, updateProfile, type AuthProfile,
} from "../lib/api";
...
/* The sidebar links to "#profile/missions" and "#profile/promo", so the page
   accepts the hash section and brings that card into view. */
const SECTION_IDS: Record<string, string> = { missions: "profile-missions", promo: "profile-promo", referral: "profile-referral" };

export function ProfilePage({ section = "", profile: initialProfile = null }: { section?: string; profile?: AuthProfile | null }) {
  const [profile, setProfile] = useState(() => ({
    displayName: initialProfile?.displayName ?? "",
    username: initialProfile?.username ?? "",
    email: initialProfile?.email ?? "",
    tier: (initialProfile?.tier ?? "FREE") as "FREE" | "PREMIUM",
    xp: initialProfile?.xp ?? 0,
    premiumUntil: initialProfile?.premiumUntil ?? null,
  }));
  ...
  // Deep links from the student sidebar ("profile/missions", "profile/promo").
  useEffect(() => {
  ...
  async function saveProfile() {
    ...
    catch (reason) { setError(errorMessage(reason, "Could not save your profile.")); }
  }
  ...
  async function saveSchool() {
    ...
    catch (reason) { setError(errorMessage(reason, "Could not save your school.")); }
  }
  ...
  async function addCourse() {
    ...
    catch (reason) { setError(errorMessage(reason, "Could not save the course.")); }
  }
  ...
  async function removeCourse(courseId: string) {
    ...
    catch (reason) { setError(errorMessage(reason, "Could not remove the course.")); }
  }
  ...
  async function confirmDeletion() {
    ...
    } catch (reason) { setError(errorMessage(reason, "Could not delete your account.")); setDeleting(false); }
  }
